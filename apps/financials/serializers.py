    ...
    def validate(self, attrs):
        ...
        if tx_type in {
            ...
            if attrs.get('category'):
                raise serializers.ValidationError({'category': 'Transfer transactions cannot use categories.'})
            if attrs.get('cash_register'):
                raise serializers.ValidationError({'cash_register': 'Transfer transactions cannot reference cash registers.'})
...
class TransferSerializer(CompanyScopedSerializer):
    ...
    from_bank_account = serializers.PrimaryKeyRelatedField(queryset=BankAccount.objects.all())
